...
export function liveControls(
  ...
  return (definitions ?? []).filter(isRequiredControl).map((definition) => {
...
/** Completed steps over total steps, across every control the playbook currently requires. */
...
export function summarizeApplication(
  ...
  const ownControls = controls.filter(
    (control) =>
      activeTicketIds.has(control.ticket_id) &&
  ...
  const required = ownControls.filter((control) => control.required);
  const stepsByControl = new Map<string, TicketControlStep[]>();
  for (const step of ownSteps) {
    stepsByControl.set(step.ticket_control_id, [
      ...(stepsByControl.get(step.ticket_control_id) ?? []),
      step,
    ]);
  }
  const derived = new Map(
    ownControls.map((control) => [
      control.id,
      liveControlStatus(control, stepsByControl.get(control.id) ?? []),
    ]),
  );
  ...
  return {
    ...
    controlsInProgress: required.filter((control) => derived.get(control.id) === "in_progress")
      .length,
    controlsCompleted: required.filter((control) => derived.get(control.id) === "completed").length,
...
export function isRequiredControl(control: ControlSummary | ControlDetail): boolean {
  return control.status === "active" && control.required;
}

/** What a ticket's progress rows should cover: every currently active, required control. */
export function reconciliationPlan(controls: ControlDetail[]): ControlReconciliation[] {
  return controls.filter(isRequiredControl).map((control) => ({
    control_id: control.control_id,
    required: true,
    step_keys: control.steps.map((step) => step.step_key),
  }));
}
